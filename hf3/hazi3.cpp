...
public:
	CheckerBoardTexture(const int width, const int height) : Texture() {
		...
		const vec4 yellow(1, 1, 0, 1), green(0, 1, 0, 1);
...
public:
	Object(Shader* _shader, Material* _material, Texture* _texture, Geometry* _geometry) :
		scale(vec3(0.3, 0.3, 0.3)), translation(vec3(0, 0, 0)), rotationAxis(0, 0, 1), dir(0,0),rotationAngle(0) {
		shader = _shader;
		texture = _texture;
		material = _material;
		geometry = _geometry;
	...
	virtual void Animate(float tstart, float tend) { 
		...
		translation = vec3(translation.x + dir.x * 0.05, translation.y + dir.y * 0.05, 1);
		if (translation.x > 6)translation.x -= 12;
		if (translation.y > 6)translation.y -= 12;
	}
};
...
public:
	void Build() {
		...
		Texture* texture = new CheckerBoardTexture(2, 2);

		Geometry* sphere = new Sphere();

		Object* tractiObject1 = new Object(phongShader, material0, texture, sphere);
		tractiObject1->translation = vec3(-5, -5, 0);
		tractiObject1->rotationAxis = vec3(0.001, 0.001, 0);
	...
	void AddBall(vec2 dir) {
		...
		Object* tractiObject = new Object(phongShader, material0, texture, sphere);
		tractiObject->translation = vec3(-5, -5, 0);
		tractiObject->rotationAxis = vec3(dir.x, dir.y, 0);
		tractiObject->dir = dir;
